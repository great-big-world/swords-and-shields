...
public class EnchantmentScreen extends HandledScreen<EnchantmentScreenHandler> {
    private static final Identifier LEVEL_TEXTURE = new Identifier(SwordsAndShields.NAMESPACE, "experience_orb");
    private static final Identifier LEVEL_DISABLED_TEXTURE = new Identifier(SwordsAndShields.NAMESPACE, "experience_orb_disabled");
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        for (int l = 0; l < handler.getEnchantmentsCount(); ++l) {
            ...
            if (o == 0) {
                ...
                context.drawGuiTexture(ENCHANTMENT_SLOT_DISABLED_TEXTURE, m, j + 14 + 19 * l + scrollOffset, 108, 19);
                ...
                if ((k < l + 1 || this.client.player.experienceLevel < o) && !this.client.player.getAbilities().creativeMode) {
                    RenderSystem.enableBlend();
                    context.drawGuiTexture(ENCHANTMENT_SLOT_DISABLED_TEXTURE, m, j + 14 + 19 * l + scrollOffset, 108, 19);
                    context.drawGuiTexture(LEVEL_DISABLED_TEXTURE, m + 6, (j + 17 + 19 * l + scrollOffset) - 1, 16, 16);
                    context.drawTextWithShadow(textRenderer, String.valueOf(l + 1), m + 2, j + 19 + 19 * l + scrollOffset, 4226832);
                    RenderSystem.disableBlend();
                    context.drawTextWrapped(this.textRenderer, stringVisitable, n, j + 16 + 19 * l + scrollOffset, p, (q & 16711422) >> 1);
                    q = 4226832;
                } else {
                    int r = mouseX - (i + 60);
                    int s = mouseY - (j + 14 + 19 * l + scrollOffset);
                    RenderSystem.enableBlend();
                    if (r >= 0 && s >= 0 && r < 108 && s < 19) {
                        context.drawGuiTexture(ENCHANTMENT_SLOT_HIGHLIGHTED_TEXTURE, m, j + 14 + 19 * l + scrollOffset, 108, 19);
                        q = 16777088;
                    } else {
                        context.drawGuiTexture(ENCHANTMENT_SLOT_TEXTURE, m, j + 14 + 19 * l + scrollOffset, 108, 19);
                    }

                    context.drawGuiTexture(LEVEL_TEXTURE, m + 6, (j + 17 + 19 * l + scrollOffset) - 1, 16, 16);
                    context.drawTextWithShadow(textRenderer, String.valueOf(l + 1), m + 2, j + 19 + 19 * l + scrollOffset, 8453920);
                    RenderSystem.disableBlend();
                    context.drawTextWrapped(this.textRenderer, stringVisitable, n, j + 16 + 19 * l + scrollOffset, p, q);
                    q = 8453920;
                }

                context.drawTextWithShadow(this.textRenderer, string, n + 86 - this.textRenderer.getWidth(string), j + 16 + 19 * l + 7 + scrollOffset, q);
            }
        }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        for (int j = 0; j < handler.getEnchantmentsCount(); ++j) {
            ...
            Enchantment enchantment = Enchantment.byRawId(this.handler.enchantmentId[j]); // null
