...
public class InGameHudMixin {
    ...
    private void gbw$hideHealthBarPre(DrawContext context, CallbackInfo ci, @Local PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideHealthHud() <= 0 && player.getStackInHand(player.getActiveHand()).isIn(SwordsAndShieldsTags.HINTS_HEALTH_HUD)) {
                float alpha = MathHelper.sin(.2f * client.world.getTime()) * .2f + .2f;
                context.setShaderColor(1f, 1f, 1f,  alpha);
            } else if (dynamicHudPlayer.gbw$getHideHealthHud() < 20)
                context.setShaderColor(1f, 1f, 1f, dynamicHudPlayer.gbw$getHideHealthHud() / 20f);
        }
    }
    ...
    private void gbw$hideHealthBarPost(DrawContext context, CallbackInfo ci, @Local PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer && dynamicHudPlayer.gbw$getHideHealthHud() < 20) {
    ...
    private void gbw$hideArmorBarPre(DrawContext context, CallbackInfo ci, @Local PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideArmorHud() <= 0 && player.getStackInHand(Hand.MAIN_HAND).isIn(SwordsAndShieldsTags.HINTS_ARMOR_HUD)) {
                float alpha = MathHelper.sin(.2f * client.world.getTime()) * .2f + .2f;
                context.setShaderColor(1f, 1f, 1f,  alpha);
            } else if (dynamicHudPlayer.gbw$getHideArmorHud() < 20)
                context.setShaderColor(1f, 1f, 1f, dynamicHudPlayer.gbw$getHideArmorHud() / 20f);
        }
    }
    ...
    private void gbw$hideArmorBarPost(DrawContext context, CallbackInfo ci, @Local PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer && dynamicHudPlayer.gbw$getHideArmorHud() < 20) {
            context.setShaderColor(1f, 1f, 1f, 1f);
        }
    }
    ...
    private void gbw$hideFoodBarPre(DrawContext context, CallbackInfo ci, @Local PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideFoodHud() <= 0 && player.getStackInHand(Hand.MAIN_HAND).isIn(SwordsAndShieldsTags.HINTS_FOOD_HUD)) {
                float alpha = MathHelper.sin(.2f * client.world.getTime()) * .2f + .2f;
                context.setShaderColor(1f, 1f, 1f,  alpha);
            } else if (dynamicHudPlayer.gbw$getHideFoodHud() < 20)
                context.setShaderColor(1f, 1f, 1f, dynamicHudPlayer.gbw$getHideFoodHud() / 20f);
        }
    }
    ...
    private void gbw$hideFoodBarPost(DrawContext context, CallbackInfo ci, @Local PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer && dynamicHudPlayer.gbw$getHideFoodHud() < 20) {
            context.setShaderColor(1f, 1f, 1f, 1f);
        }
    }
    ...
    private void gbw$hideExpBarPre(DrawContext context, float tickDelta, CallbackInfo ci) {
        if (client.player instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideExpHud() <= 0 && client.player.getStackInHand(Hand.MAIN_HAND).isIn(SwordsAndShieldsTags.HINTS_EXPERIENCE_HUD)) {
                float alpha = MathHelper.sin(.2f * client.world.getTime()) * .2f + .2f;
                context.setShaderColor(1f, 1f, 1f,  alpha);
            } else if (dynamicHudPlayer.gbw$getHideExpHud() < 20)
                context.setShaderColor(1f, 1f, 1f, dynamicHudPlayer.gbw$getHideExpHud() / 20f);
        }
    }
    ...
    private void gbw$repositionFoodForHiddenExp(DrawContext instance, Identifier texture, int x, int y, int width, int height, @Local(argsOnly = true) PlayerEntity player) {
        if (player instanceof DynamicHudPlayer dynamicHudPlayer && dynamicHudPlayer.gbw$getHideExpHud() < 0) {
    ...
    private static void gbw$repositionArmorForHiddenExp(DrawContext instance, Identifier texture, int x, int y, int width, int height, @Local(argsOnly = true) PlayerEntity player) {
        int offset = 0;
        if (player instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideExpHud() < 0)
                offset += 7;
            if (dynamicHudPlayer.gbw$getHideHealthHud() < 0)
                offset += 10;
        }
    ...
    private void gbw$repositionBubblesForHiddenExp(DrawContext instance, Identifier texture, int x, int y, int width, int height, @Local PlayerEntity playerEntity) {
        int offset = 0;
        if (playerEntity instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideExpHud() < 0)
                offset += 7;
            if (dynamicHudPlayer.gbw$getHideFoodHud() < 0)
                offset += 10;
        }
        instance.drawGuiTexture(texture, x, y + offset, width, height);
    }
    ...
    private int gbw$repositionHeldItemNameForHiddenHud(DrawContext instance, TextRenderer textRenderer, Text text, int x, int y, int color) {
        ...
        if (client.player instanceof DynamicHudPlayer dynamicHudPlayer) {
            if (dynamicHudPlayer.gbw$getHideHealthHud() < 0 && dynamicHudPlayer.gbw$getHideFoodHud() < 0)
                offset += 7;
            if (dynamicHudPlayer.gbw$getHideArmorHud() < 0)
                offset += 10;
        }
